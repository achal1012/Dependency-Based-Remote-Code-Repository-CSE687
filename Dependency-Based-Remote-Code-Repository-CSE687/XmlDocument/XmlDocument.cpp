...
XmlDocument buildDocument()
{
  ...
  sPtr checkInStatus = makeTaggedElement("checkInStatus");
  checkInStatus->addChild(makeTextElement("closed"));
  pRoot->addChild(checkInStatus);
  return doc;
}
...
{
  std::cout << "\nMY TEST:\n";
  std::string testTag = "checkInStatus";
...
int main()
{
  ...
  std::vector<sPtr> foundStatus = doc.element(testStatus).select();
  if (foundStatus.size() > 0) {
	  if (foundStatus[0]->tag() == "closed")
		  std::cout << "\ncheck in is closed";
	  else if (foundStatus[0]->tag() == "open")
		  std::cout << "\ncheck in is open";
	  else
		  std::cout << foundStatus[0]->tag();
		  std::cout << "\nwrong data in checkInStatus tag";
  }
  else
	  std::cout << "checkInStatus tag not found!";
  ////////////////////////////////
